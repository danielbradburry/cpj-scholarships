...
export class ApplicationComponent implements OnInit {
  ...
  submitForm(form) {
    ...
    this.scholarshipService.updateFormResponse(this.program.url, this.scholarship.url, this.currentStep.form.applicationFormID, {
  ...
  finalSubmit() {
    this.confirmService.confirm('Warning: Submitting your application is final and the information cannot be altered after this is done. Are you sure?', () => {
      this.submitting = true;

      this.scholarshipService.submitApplication(this.program.url, this.scholarship.url)
        .pipe(takeUntil(this.unsubscribe))
        .subscribe((response: any) => {
          if (response.valid) {
            this.router.navigate(['/'], { queryParamsHandling: 'preserve' });
