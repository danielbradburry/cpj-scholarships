...
@Injectable()
export class ThirdPartyRequestResolver implements Resolve<any> {
  ...
  resolve(route: ActivatedRouteSnapshot) {
    ...
    return this.scholarshipService
      .getThirdPartySubmissionRequest(scholarshipProgramKey, route.params.scholarshipURL, route.params.applicationID, route.params.requestID)
      .pipe(
        catchError((error) => {
          if (error.status === 404) {
